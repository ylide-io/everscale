        ...
        const result = await this.gqlQueryMessages(`
			query {
				...
				filter: {
					...
					${_at
            ? `created_at: { ${created_at.lt !== undefined
                ? `lt: "${moment_1.default.unix(created_at.lt).utc().toISOString()}", `
                : ''} ${created_at.gt !== undefined
                ? `gt: "${moment_1.default.unix(created_at.gt).utc().toISOString()}", `
                : ''} }, `
            : ''}
					${_lt
            ? `created_lt: { ${created_lt.lt !== undefined ? `lt: "${'0x' + created_lt.lt.toString(16)}", ` : ''} ${created_lt.gt !== undefined ? `gt: "${'0x' + created_lt.gt.toString(16)}", ` : ''} }, `
            : ''}
				}
