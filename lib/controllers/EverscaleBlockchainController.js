...
class EverscaleBlockchainController extends sdk_1.AbstractBlockchainController {
    ...
    constructor(options = {}) {
        ...
        this.broadcasterContractAddress =
            options.broadcasterContractAddress || (options.dev ? constants_1.DEV_BROADCASTER_ADDRESS : constants_1.BROADCASTER_ADDRESS);
    ...
    async getBalance(address) {
        const stringValue = await this.ever.getBalance(new everscale_inpage_provider_1.Address(address));
        return {
    ...
    async extractPublicKeyFromAddress(address) {
        ...
        return {
            keyVersion: sdk_2.YlidePublicKeyVersion.INSECURE_KEY_V1,
            publicKey: sdk_1.PublicKey.fromBytes(sdk_1.PublicKeyType.YLIDE, rawKey),
            timestamp: 0,
        };
    }
    async _retrieveMessageHistoryByTime(contractAddress, subject, fromTimestamp, toTimestamp, limit) {
        await core_1.default.ensureNekotonLoaded();
        if (!contractAddress) {
        ...
        const result = events.map(m => subject.type === sdk_1.BlockchainSourceType.DIRECT
    ...
    async _retrieveMessageHistoryByBounds(contractAddress, subject, fromMessage, toMessage, limit) {
        await core_1.default.ensureNekotonLoaded();
        const events = await this.queryMessagesList(contractAddress, subject, limit, {
    ...
    async retrieveMessageHistoryByTime(sender, recipient, fromTimestamp, toTimestamp, limit) {
        ...
        return this._retrieveMessageHistoryByTime(mailerAddress, { type: sdk_1.BlockchainSourceType.DIRECT, sender, recipient }, fromTimestamp, toTimestamp, limit);
    }
    async retrieveMessageHistoryByBounds(sender, recipient, fromMessage, toMessage, limit) {
        const mailerAddress = this.getDefaultMailerAddress();
        return this._retrieveMessageHistoryByBounds(mailerAddress, { type: sdk_1.BlockchainSourceType.DIRECT, sender, recipient }, fromMessage, toMessage, limit);
    }
    async retrieveBroadcastHistoryByTime(sender, fromTimestamp, toTimestamp, limit) {
        ...
        return this._retrieveMessageHistoryByTime(broadcasterAddress, { type: sdk_1.BlockchainSourceType.BROADCAST, sender }, fromTimestamp, toTimestamp, limit);
    }
    async retrieveBroadcastHistoryByBounds(sender, fromMessage, toMessage, limit) {
        const broadcasterAddress = this.broadcasterContractAddress;
        return this._retrieveMessageHistoryByBounds(broadcasterAddress, { type: sdk_1.BlockchainSourceType.BROADCAST, sender }, fromMessage, toMessage, limit);
    }
    ...
    async retrieveMessageContentByMsgId(msgId) {
        ...
        let messages = await this.gqlQueryMessages((0, misc_2.getContractMessagesQuery)(fakeAddress, this.mailerContractAddress), {});
        if (!messages.length) {
            messages = await this.gqlQueryMessages((0, misc_2.getContractMessagesQuery)(fakeAddress, this.broadcasterContractAddress), {});
        }
        if (!messages.length) {
            return null;
        }
    ...
    formatBroadcastMessage(sender, message) {
        const body = (0, contractUtils_1.decodeBroadcastMessageBody)(message.body);
        return {
            isBroadcast: true,
            msgId: body.msgId,
            createdAt: message.created_at,
    ...
    async queryMessagesList(contractAddress, subject, limit, filter, nextPageAfterMessage) {
        const address = subject.type === sdk_1.BlockchainSourceType.DIRECT
            ? subject.recipient
                ? (0, misc_1.uint256ToAddress)(subject.recipient, true, true)
                : null
            : subject.sender
                ? subject.sender
                : null;
        ...
        let result;
        if (subject.type === sdk_1.BlockchainSourceType.DIRECT) {
            ...
            result = await this.gqlQueryMessages(`
                ...
                : ''}
							${_lt
                ? `created_lt: { ${createdLt.lt !== undefined
        ...
        if (limit && result.length === limit) {
            ...
            if (result.length === 0) {
                ...
                const after = await this.queryMessagesList(contractAddress, subject, limit ? limit - result.length : undefined, filter, result[result.length - 1]);
