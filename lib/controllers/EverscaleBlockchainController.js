...
class EverscaleBlockchainController extends sdk_1.AbstractBlockchainController {
    ...
    async _retrieveMessageHistoryByTime(contractAddress, subject, fromTimestamp, toTimestamp, limit) {
        ...
        const result = events.map(m => subject.type === sdk_1.BlockchainSourceSubjectType.RECIPIENT
    ...
    async retrieveBroadcastHistoryByTime(sender, fromTimestamp, toTimestamp, limit) {
        ...
        return this._retrieveMessageHistoryByTime(broadcasterAddress, { type: sdk_1.BlockchainSourceSubjectType.AUTHOR, address: sender }, fromTimestamp, toTimestamp, limit);
    }
    async retrieveBroadcastHistoryByBounds(sender, fromMessage, toMessage, limit) {
        const broadcasterAddress = this.broadcasterContractAddress;
        return this._retrieveMessageHistoryByBounds(broadcasterAddress, { type: sdk_1.BlockchainSourceSubjectType.AUTHOR, address: sender }, fromMessage, toMessage, limit);
    }
    ...
    async queryMessagesList(contractAddress, subject, limit, filter, nextPageAfterMessage) {
        ...
        let result;
        if (subject.type === sdk_1.BlockchainSourceSubjectType.RECIPIENT) {
