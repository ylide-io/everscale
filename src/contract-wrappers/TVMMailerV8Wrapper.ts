...
export class TVMMailerV8Wrapper {
	...
	> {
		options?.log.log('MailerV8 retrieveHistoryAscRaw external start');
		const result = await this.cache.contractOperation(mailer, async (contract, ever, gql, core) => {
			options?.log.log('MailerV8 retrieveHistoryAscRaw internal start');
			...
			options?.log.log('MailerV8 retrieveHistoryAscRaw internal end');
			return internalResult;
		});
		options?.log.log('MailerV8 retrieveHistoryAscRaw external end');
		return result;
	}
