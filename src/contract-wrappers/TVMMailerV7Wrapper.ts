...
export class TVMMailerV7Wrapper {
	...
	> {
		options?.log.log('MailerV7 retrieveHistoryAscRaw external start');
		const result = await this.cache.contractOperation(mailer, async (contract, ever, gql, core) => {
			options?.log.log('MailerV7 retrieveHistoryAscRaw internal start');
			...
			options?.log.log('MailerV7 retrieveHistoryAscRaw internal end');
			return internalResult;
		});
		options?.log.log('MailerV7 retrieveHistoryAscRaw external end');
		return result;
	}
